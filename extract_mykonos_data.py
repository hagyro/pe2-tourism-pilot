"""Συγκέντρωση όλων των δεδομένων Χανίων → assets/data/mykonos.json
...
OUT  = ROOT / "working_folder/output/html_companion/assets/data/mykonos.json"
...
def load_arrivals():
    ...
    chania = df[df["destination_gr"] == "Μύκονος"].sort_values("year").reset_index(drop=True)
    dem = pd.read_csv(ROOT / "data_v2/analysis/indicators_demand.csv", encoding="utf-8-sig")
    dem_c = dem[dem["destination_gr"] == "Μύκονος"]
...
def load_monthly():
    ...
    sub = df[(df["nuts_code"] == "EL42") & (df["year"] == 2024) &
...
def load_seasonality():
    ...
    chania = df[df["destination_gr"] == "Μύκονος"].sort_values("year")
    return {
        "years": chania["year"].tolist(),
...
def load_rooms_by_stars():
    ...
    sub = df[(df["destination_gr"] == "Μύκονος") & (df["year"] == 2024) & (df["star_category"] != "Total")]
    return {
...
def load_mrb_overall():
    ...
    chania = df[df["destination"] == "ΜΥΚΟΝΟΣ"].copy()
    out = {}
...
def load_quotes():
    md = (ROOT / "working_folder/output/chapters_v2/06_ΜΥΚΟΝΟΣ_v2.md").read_text(encoding="utf-8")
    quotes = []
...
def main():
    print(f"Συλλογή δεδομένων Μυκόνου → {OUT}")
    bundle = {
        "destination": {
            "code": "ΜΥΚΟΝΟΣ", "display": "Μύκονος",
            "nuts3": "EL422", "nuts2": "EL42",
            "region": "Νότιο Αιγαίο",
            "subtitle": "Δήμος Μυκόνου — Τουριστικό Προφίλ 2030–2035",
            "population_2021": 10704,
            "population_municipality": 10704,
        },
        "kpis": {
            "arrivals_2024": 557339,
            "tfi_2024": 158.0,
            "tii_2024": 52.1,
            "poi_theta": 0.82,
            "q9_sat_mean": 3.64,
            "alos_2024": 3.43,
            "beds_2024": 17000,
            "str_overnight_share_2024": 35.0,
            "adr_eur_2024": 607,
            "blue_flag_2024": 5,
            "tourism_employment_pct": 89.3,
            "natura_areas_count": 4,
            "gva_gi_share_2023": 65.0,
        },
