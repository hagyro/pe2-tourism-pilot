"""Συγκέντρωση όλων των δεδομένων Χανίων → assets/data/chania.json
...
OUT  = ROOT / "working_folder/output/html_companion/assets/data/chania.json"
...
def load_arrivals():
    ...
    chania = df[df["destination_gr"] == "Χανιά"].sort_values("year").reset_index(drop=True)
    dem = pd.read_csv(ROOT / "data_v2/analysis/indicators_demand.csv", encoding="utf-8-sig")
    dem_c = dem[dem["destination_gr"] == "Χανιά"]
...
def load_monthly():
    ...
    sub = df[(df["nuts_code"] == "EL43") & (df["year"] == 2024) &
...
def load_capacity():
    ...
    chania = df[df["destination_gr"] == "Chania"].sort_values("year")
    return {
        "years": chania["year"].tolist(),
...
def load_mrb_overall():
    ...
    chania = df[df["destination"] == "ΧΑΝΙΑ"].copy()
    out = {}
...
def load_quotes():
    md = (ROOT / "working_folder/output/chapters_v2/04_ΧΑΝΙΑ_v2.md").read_text(encoding="utf-8")
    quotes = []
...
def main():
    print(f"Συλλογή δεδομένων Χανίων → {OUT}")
    bundle = {
        "destination": {
            "code": "ΧΑΝΙΑ", "display": "Χανιά",
            "nuts3": "EL434", "nuts2": "EL43",
            "region": "Κρήτη",
            "subtitle": "Δυτική Κρήτη — Τουριστικό Προφίλ 2030–2035",
            "population_2021": 156706,
        },
        "kpis": {
            "arrivals_2024": 1359014,
            "tfi_2024": 12.72,
            "tii_2024": 8.67,
            "poi_theta": 0.54,
            "q9_sat_mean": 3.61,
            "alos_2024": 5.35,
            "beds_2024": 51917,
            "str_overnight_share_2024": 24.8,
            "blue_flag_2024": 43,
            "permits_2024": 515,
            "tourism_employment_pct": 46.7,
            "land_artificial_pct_2022": 4.65,
            "gva_gi_share_2023": 37,
        },
